...
SRC_SUB_DIRS=". audio events gfx gfx/hqx map map/2d map/3d script ui"
...
# link against a2elight (note: floor debug lib is suffixed by "d")
if [ $BUILD_MODE == "debug" ]; then
	if [ $BUILD_OS == "mingw" ]; then
		LDFLAGS="${LDFLAGS} -la2elightd_static"
	else
		LDFLAGS="${LDFLAGS} -la2elightd"
	fi
else
	if [ $BUILD_OS == "mingw" ]; then
		LDFLAGS="${LDFLAGS} -la2elight_static"
	else
		LDFLAGS="${LDFLAGS} -la2elight"
	fi
fi
...
# a2elight include folder (either should work)
INCLUDES="${INCLUDES} -isystem /usr/include/a2elight -isystem /usr/local/include/a2elight"
...
WARNINGS="${WARNINGS} -Wno-documentation-unknown-command -Wno-cast-align"
if [ ${BUILD_ARCH_SIZE} == "x32" ]; then
